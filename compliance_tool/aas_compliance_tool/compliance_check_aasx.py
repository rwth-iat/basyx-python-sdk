...
def check_aas_example(file_path: str, state_manager: ComplianceToolStateManager, **kwargs) -> None:
    ...
    if checker2.check(
        isinstance(cp_new.created, datetime.datetime),
        "core property created must be of type datetime",
    ...
    checker2.check(
        cp_new.description == cp.description,
        "description must be {}".format(cp.description),
        description=cp_new.description,
    )
    checker2.check(
        cp_new.lastModifiedBy == cp.lastModifiedBy,
        "lastModifiedBy must be {}".format(cp.lastModifiedBy),
        lastModifiedBy=cp_new.lastModifiedBy,
    )

    if checker2.check(
        isinstance(cp_new.modified, datetime.datetime),
        "modified must be of type datetime",
...
def check_aasx_files_equivalence(
    ...
    checker2.check(
        cp_1.lastModifiedBy == cp_2.lastModifiedBy,
        "lastModifiedBy must be {}".format(cp_1.lastModifiedBy),
    ...
    for file_name in files_1:
        both_contain = file_checker.check(
            file_name in files_2, "second file must contain supplementary file {}".format(file_name)
        )
        if both_contain:
            expected_type = files_1.get_content_type(file_name)
            file_checker.check(
                expected_type == files_2.get_content_type(file_name),
                f"second file must contain supplementary file {file_name} with content-type {{expected_type}}",
                content_type=files_2.get_content_type(file_name),
            )
            expected_checksum = files_1.get_sha256(file_name)
            file_checker.check(
                expected_checksum == files_2.get_sha256(file_name),
                f"second file must contain supplementary file {file_name} with sha256 {expected_checksum.hex()}",
                checksum=files_2.get_sha256(file_name).hex(),
            )

    for file_name in files_2:
        file_checker.check(file_name in files_1, "first file must contain supplementary file {}".format(file_name))
