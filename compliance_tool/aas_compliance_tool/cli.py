...
from aas_compliance_tool import (
    compliance_check_xml as compliance_tool_xml,
...
def parse_cli_arguments() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "action",
        choices=["create", "c", "deserialization", "d", "example", "e", "files", "f"],
    ...
    parser.add_argument(
        "file_2", nargs="?", default=None, help="path to file 2: is required if action f or files is chosen"
    )
    parser.add_argument(
        "-v",
        "--verbose",
        help="Print detailed information for each check. Multiple -v options "
...
def main():
    ...
    data_checker_kwargs = {"check_extensions": args.dont_check_extensions}
    ...
    if args.action == "create" or args.action == "c":
            ...
            if args.aasx:
                with aasx.AASXWriter(args.file_1) as writer:
                    ...
                    writer.write_aas_objects(
                        "/aasx/data.json" if args.json else "/aasx/data.xml",
        ...
        if args.file_2:
            if args.aasx:
                compliance_tool_aasx.check_aasx_files_equivalence(
                    args.file_1, args.file_2, manager, **data_checker_kwargs
                )
            elif args.json:
                compliance_tool_json.check_json_files_equivalence(
                    args.file_1, args.file_2, manager, **data_checker_kwargs
                )
            elif args.xml:
                compliance_tool_xml.check_xml_files_equivalence(
                    args.file_1, args.file_2, manager, **data_checker_kwargs
                )
        else:
