...
class ComplianceToolXmlTest(unittest.TestCase):
    ...
    def test_check_example_success(
        self, mock_data_checker: mock.MagicMock, mock_read_xml_file: mock.MagicMock, mock_open: mock.MagicMock
    ) -> None:
        manager = ComplianceToolStateManager()
    ...
    def test_check_example_fail_on_read(
        ...
        mock_read_xml_file.side_effect = create_mock_effect(
            "basyx.aas.adapter.xml.xml_deserialization", "error", error_msg="Error on reading aas xml file!"
        )
        compliance_tool.check_aas_example("", manager)
    ...
    @mock.patch("basyx.aas.adapter.xml.xml_deserialization.read_aas_xml_file", autospec=True)
    @mock.patch("aas_compliance_tool.compliance_check_xml.AASDataChecker", autospec=True)
    def test_check_xml_files_equivalence_file1_fail_on_deserialization(
        self, mock_data_checker, mock_read_xml_file, mock_open
    ) -> None:
        manager = ComplianceToolStateManager()
    ...
    @mock.patch("basyx.aas.adapter.xml.xml_deserialization.read_aas_xml_file", autospec=True)
    @mock.patch("aas_compliance_tool.compliance_check_xml.AASDataChecker", autospec=True)
    def test_check_xml_files_equivalence_file2_fail_on_deserialization(
        self, mock_data_checker, mock_read_xml_file, mock_open
    ) -> None:
        manager = ComplianceToolStateManager()
    ...
    @mock.patch("basyx.aas.adapter.xml.xml_deserialization.read_aas_xml_file", autospec=True)
    @mock.patch("aas_compliance_tool.compliance_check_xml.AASDataChecker", autospec=True)
    def test_check_xml_files_equivalence_fail_on_check(
        self, mock_data_checker: mock.MagicMock, mock_read_xml_file, mock_open
    ) -> None:
        manager = ComplianceToolStateManager()
