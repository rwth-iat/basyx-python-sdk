"""
This script dynamically verifies that the modules respect the versions defined in the global versions.yaml.
...
def check_python(versions: dict[str, Any], pyproject_paths: list[Path], repo_root: Path) -> list[str]:
    ...
    if not min_version:
        return ["versions.yaml: missing `python.min`"]
    ...
    for path in pyproject_paths:
        ...
        for tool, version in tools.items():
            ...
            if not any(pattern.match(dep) for dep in deps):
                errors.append(f"{display}: dev dependency `{tool}=={version}` missing "
...
def check_dockerfiles(versions: dict[str, Any], dockerfile_paths: list[Path], repo_root: Path) -> list[str]:
    ...
    if not min_str or not max_str:
        return ["versions.yaml: `python.min` / `python.max` required for Dockerfile checks"]
    ...
    for path in dockerfile_paths:
        ...
        if found < min_ver or found > max_ver:
            errors.append(f"{display}: base image `python:{found_str}` outside supported "
...
def check_configs(versions: dict[str, Any], config_paths: list[Path], repo_root: Path) -> list[str]:
    ...
    contains `<name>`. Config files are loaded as YAML.
    """
    ...
    for module_name, expected_entries in config_section.items():
        ...
        if not matches:
            errors.append(f"config.{module_name}: no config file in `configuration_locations.configs` "
                          f"has `{module_name}` in its path")
            continue
        if len(matches) > 1:
            errors.append(f"config.{module_name}: ambiguous — multiple config files match "
        ...
        for key, expected in expected_entries.items():
            actual = loaded.get(key) if isinstance(loaded, dict) else None
            if actual is None:
...
def main(versions_yaml_path: Path) -> int:
    versions = _load_yaml(versions_yaml_path)
    repo_root = versions_yaml_path.resolve().parent
    ...
    print(f"Success: `{versions_yaml_path.name}` coincides with "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--versions",
                        type=Path,
                        default=Path(__file__).resolve().parents[2] / "versions.yaml",
