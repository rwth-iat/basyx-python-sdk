...
class CheckResult(NamedTuple):
    ...
    def __repr__(self):
        return "{}: {} ({})".format(
            ...
            self.expectation,
            ", ".join(
                "{}={}".format(k, pprint.pformat(v, depth=2, width=2**14, compact=True)) for k, v in self.data.items()
            ),
        )
    ...
    def raise_failed(self) -> None:
        ...
        if len(failed) > 0:
            raise AssertionError(
                "{} of {} checks failed".format(len(failed), len(self.checks)), [f.expectation for f in failed]
            )
...
class AASDataChecker(DataChecker):
    ...
    def _check_has_semantics_equal(self, object_: model.HasSemantics, expected_object: model.HasSemantics):
        ...
        for suppl_semantic_id in expected_object.supplemental_semantic_id:
            ...
            self.check(
                given_semantic_id is not None, f"{object_!r} must have supplementalSemanticId", value=suppl_semantic_id
            )

        found_elements = self._find_extra_object(
            object_.supplemental_semantic_id, expected_object.supplemental_semantic_id, model.Reference
        )
        self.check(
            found_elements == set(),
    ...
    def _check_has_data_specification_equal(
        ...
        found_elements = self._find_extra_elements_by_attribute(
            object_.embedded_data_specifications, expected_object.embedded_data_specifications, "data_specification"
        )
        self.check(
            found_elements == set(),
    ...
    def _check_submodel_elements_equal_unordered(
        ...
        self.check(
            found_elements == set(), "{} must not have extra elements".format(repr(object_)), value=found_elements
        )
    ...
    def check_submodel_element_collection_equal(
        self, object_: model.SubmodelElementCollection, expected_value: model.SubmodelElementCollection
    ):
        """
    ...
    def check_annotated_relationship_element_equal(
        self, object_: model.AnnotatedRelationshipElement, expected_value: model.AnnotatedRelationshipElement
    ):
        """
        ...
        self.check_contained_element_length(object_, "annotation", model.DataElement, len(expected_value.annotation))
    ...
    def _find_extra_object(self, object_list: Iterable, search_list: Iterable, type_) -> Union[Set, None]:
        """
    ...
    def check_entity_equal(self, object_: model.Entity, expected_value: model.Entity):
        ...
        self.check(
            found_elements == set(), f"Entity {repr(object_)} must not have extra statements", value=found_elements
        )

    def _check_specific_asset_ids_equal(
        self, object_: Iterable[model.SpecificAssetId], expected_value: Iterable[model.SpecificAssetId], object_parent
    ):
        ...
        self.check(
            found_elements == set(), f"{repr(object_parent)} must not have extra specificAssetIds", value=found_elements
        )
    ...
    def check_specific_asset_id(self, object_: model.SpecificAssetId, expected_value: model.SpecificAssetId):
        """
    ...
    def check_asset_information_equal(self, object_: model.AssetInformation, expected_value: model.AssetInformation):
        ...
        self.check_attribute_equal(object_, "global_asset_id", expected_value.global_asset_id)
        self.check_contained_element_length(
            object_, "specific_asset_id", model.SpecificAssetId, len(expected_value.specific_asset_id)
        )
        self._check_specific_asset_ids_equal(object_.specific_asset_id, expected_value.specific_asset_id, object_)
    ...
    def check_asset_administration_shell_equal(
        self, object_: model.AssetAdministrationShell, expected_value: model.AssetAdministrationShell
    ):
        """
        ...
        self.check(
            found_elements == set(),
            "Asset Administration Shell {} must not have extra submodel references".format(repr(object_)),
            value=found_elements,
        )
    ...
    def check_concept_description_equal(
        ...
        self._check_identifiable_equal(object_, expected_value)
        self._check_has_data_specification_equal(object_, expected_value)
        self.check_contained_element_length(object_, "is_case_of", model.Reference, len(expected_value.is_case_of))
        ...
        found_elements = self._find_extra_object(object_.is_case_of, expected_value.is_case_of, model.ModelReference)
        self.check(
            found_elements == set(),
            "Concept Description Reference {} must not have extra is case of references".format(repr(object_)),
            value=found_elements,
        )

    def check_data_specification_content_equal(
        self, object_: model.DataSpecificationContent, expected_value: model.DataSpecificationContent
    ):
        """
    ...
    def _check_data_specification_iec61360_equal(
        self, object_: model.base.DataSpecificationIEC61360, expected_value: model.base.DataSpecificationIEC61360
    ):
        """
    ...
    def _check_value_list_equal(self, object_: model.ValueList, expected_value: model.ValueList):
        ...
        self.check(found_elements == set(), "ValueList must not have extra ValueReferencePairs", value=found_elements)

    def check_identifiable_store(
        self, identifiable_store_1: model.DictIdentifiableStore, identifiable_store_2: model.DictIdentifiableStore
    ):
        """
        ...
        for shell_2 in shell_list_2:
            ...
            if self.check(
                shell_1 is not None,
                "Asset administration shell {} must exist in given asset administrationshell list".format(shell_2),
            ):
        ...
        self.check(
            found_elements == set(),
            "Given asset administration shell list must not have extra asset administration shells",
            value=found_elements,
        )
        ...
        self.check(found_elements == set(), "Given submodel list must not have extra submodels", value=found_elements)

        for cd_2 in concept_description_list_2:
            cd_1 = identifiable_store_1.get(cd_2.id)
            if self.check(
                cd_1 is not None, "Concept description {} must exist in given concept description list".format(cd_2)
            ):
    ...
    def check_element_in(self, object_: model.Referable, parent: object, **kwargs) -> bool:
        ...
        """
        return self.check(
            object_.parent == parent, "{} must exist in {}s".format(repr(object_), repr(parent)), **kwargs
        )
    ...
    def check_contained_element_length(
        ...
        return self.check(
            count == length,
            "Attribute {} of {} must contain {} {}s".format(attribute_name, repr(object_), length, class_name.__name__),
            **kwargs,
        )
