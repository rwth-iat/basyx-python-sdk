...
class CouchDBIdentifiableStore(model.AbstractObjectStore[model.Identifier, model.Identifiable]):
    ...
    def get_identifiable_by_couchdb_id(self, couchdb_id: str) -> model.Identifiable:
        ...
        if not isinstance(obj, model.Identifiable):
            raise CouchDBResponseError(
                "The CouchDB document with id {} does not contain an identifiable AAS object.".format(couchdb_id)
            )
        set_couchdb_revision(
            "{}/{}/{}".format(self.url, self.database_name, urllib.parse.quote(couchdb_id, safe="")), data["_rev"]
        )
    ...
    def add(self, x: model.Identifiable) -> None:
        ...
        try:
            ...
            set_couchdb_revision(
                "{}/{}/{}".format(self.url, self.database_name, self._transform_id(x.id)), response["rev"]
            )
        except CouchDBServerError as e:
            if e.code == 409:
    ...
    def commit(self, x: model.Identifiable) -> None:
        ...
        try:
            response = self._do_request(
                "{}?rev={}".format(doc_url, rev), "PUT", {"Content-type": "application/json"}, data.encode("utf-8")
            )
            ...
            if e.code == 404:
                ...
                raise CouchDBConflictError(
                    "Object with id {} has been modified in the database since it was last fetched.".format(x.id)
                ) from e
            raise
    ...
    def discard(self, x: model.Identifiable, safe_delete=False) -> None:
        ...
        rev = get_couchdb_revision("{}/{}/{}".format(self.url, self.database_name, self._transform_id(x.id)))
        ...
        try:
            self._do_request(
                "{}/{}/{}?rev={}".format(self.url, self.database_name, self._transform_id(x.id), rev), "DELETE"
            )
        except CouchDBServerError as e:
            if e.code == 404:
                raise KeyError("No AAS object with id {} exists in CouchDB database".format(x.id)) from e
            ...
            raise
        delete_couchdb_revision("{}/{}/{}".format(self.url, self.database_name, self._transform_id(x.id)))
        with self._object_cache_lock:
            del self._object_cache[x.id]

    @classmethod
    def _do_request(
        cls, url: str, method: str = "GET", additional_headers: Dict[str, str] = {}, body: Optional[bytes] = None
        ...
        headers = urllib3.make_headers(
            keep_alive=True, accept_encoding=True, basic_auth="{}:{}".format(*auth) if auth else None
        )
        ...
        if not (200 <= response.status < 300):
            logger.debug("Request %s %s finished with HTTP status code %s.", method, url, response.status)
            ...
            try:
                ...
                raise CouchDBResponseError("Could not parse error message of HTTP {}".format(response.status))
    ...
    def __contains__(self, x: object) -> bool:
        ...
        try:
            self._do_request("{}/{}/{}".format(self.url, self.database_name, self._transform_id(identifier)), "HEAD")
        except CouchDBServerError as e:
            if e.code == 404:
                return False
            raise
