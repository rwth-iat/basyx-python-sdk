...
class LangStringSet(MutableMapping[str, str]):
    ...
    def _check_language_tag_constraints(cls, ltag: str):
        ...
        if re.match(pattern, ltag) is None:
            raise ValueError(f"The language tag must follow the format defined in BCP 47. Given language tag: {ltag}")
    ...
    def validate_id_short(cls, id_short: NameType) -> None:
        ...
        **Constraint AASd-002:** idShort shall only feature letters, digits, underscore (``_``), hyphen (``-``);
        ...
        if not re.fullmatch("[A-Za-z0-9_-]*", test_id_short):
            raise AASConstraintViolation(2, "The id_short must contain only letters, digits underscore and hyphen")
        ...
        if self.parent is not None:
            if id_short is None:
                raise AASConstraintViolation(
                    117, f"id_short of {self!r} cannot be unset, since it is already contained in {self.parent!r}"
                )
            from .submodel import SubmodelElementList
            ...
            for set_ in self.parent.namespace_element_sets:
                if set_.contains_id("id_short", id_short):
                    raise AASConstraintViolation(
                        22, "Object with id_short '{}' is already present in the parent Namespace".format(id_short)
                    )
    ...
    **Constraint AASd-124:** For external references, i.e. References with Reference/type = ExternalReference,
    ...
    def __init__(self, key: Tuple[Key, ...], referred_semantic_id: Optional["Reference"] = None):
        ...
        if not key[0].type.is_generic_globally_identifiable:
            raise AASConstraintViolation(
                122,
                f"The type of the first key of an ExternalReference must be a GenericGloballyIdentifiable: {key[0]!r}",
            )
    ...
    **Constraint AASd-128:** For model references the Key/value of a Key preceded by a Key with
    ...
    def __init__(self, key: Tuple[Key, ...], type_: Type[_RT], referred_semantic_id: Optional[Reference] = None):
        ...
        if not key[0].type.is_aas_identifiable:
            raise AASConstraintViolation(
                123, f"The type of the first key of a ModelReference must be an AasIdentifiable: {key[0]!r}"
            )
        for k in key[1:]:
        ...
        if not key[-1].type.is_generic_fragment_key:
            for k in key[:-1]:
                if k.type.is_generic_fragment_key:
                    raise AASConstraintViolation(
                        126, f"Key {k!r} is a GenericFragmentKey, but the last key of the chain is not: {key[-1]!r}"
                    )
    ...
    def resolve(self, provider_: "provider.AbstractObjectProvider") -> _RT:
        ...
        if not isinstance(item, self.type):
            raise UnexpectedTypeError(
                item, "Retrieved object {} is not an instance of referenced type {}".format(item, self.type.__name__)
            )
        return item
    ...
    def get_identifier(self) -> Identifier:
        ...
        """
        try:
            last_identifier = next(key.get_identifier() for key in reversed(self.key) if key.get_identifier())
        ...
        if self.parent is not None:
            for set_ in self.parent.namespace_element_sets:
                if set_.contains_id("name", name):
                    raise KeyError("Object with name '{}' is already present in the parent Namespace".format(name))
            set_add_list: List[NamespaceSet] = []
            for set_ in self.parent.namespace_element_sets:
                if self in set_:
                    set_add_list.append(set_)
        ...
        if self.parent is not None:
            for set_ in self.parent.namespace_element_sets:
                if set_.contains_id("type", type_):
                    raise KeyError("Object with type '{}' is already present in the parent Namespace".format(type_))
            set_add_list: List[NamespaceSet] = []
            for set_ in self.parent.namespace_element_sets:
                if self in set_:
                    set_add_list.append(set_)
...
class NamespaceSet(MutableSet[_NSO], Generic[_NSO]):
    ...
    def _check_value_is_not_in_backend(
        self, element: _NSO, attr_name: str, attr, backend_dict: Dict[ATTRIBUTE_TYPES, _NSO], set_: "NamespaceSet"
    ):
    ...
    def __repr__(self) -> str:
        return (
            ...
            "semantic_id={}, supplemental_semantic_id={})".format(
                self.name, self.value, self.external_subject_id, self.semantic_id, self.supplemental_semantic_id
            )
        )
