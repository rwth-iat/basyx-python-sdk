...
def _generate_element(name: str, text: Optional[str] = None, attributes: Optional[Dict] = None) -> etree._Element:
    """
...
def specific_asset_id_to_xml(obj: model.SpecificAssetId, tag: str = NS_AAS + "specificAssetId") -> etree._Element:
    """
...
def reference_element_to_xml(obj: model.ReferenceElement, tag: str = NS_AAS + "referenceElement") -> etree._Element:
    """
...
def submodel_element_list_to_xml(
    ...
    et_submodel_element_list.append(
        _generate_element(
            NS_AAS + "typeValueListElement", _generic.KEY_TYPES[model.KEY_TYPES_CLASSES[obj.type_value_list_element]]
        )
    )
    if obj.value_type_list_element is not None:
        et_submodel_element_list.append(
            _generate_element(
                NS_AAS + "valueTypeListElement", model.datatypes.XSD_TYPE_NAMES[obj.value_type_list_element]
            )
        )
    if len(obj.value) > 0:
        et_value = _generate_element(NS_AAS + "value")
...
def basic_event_element_to_xml(obj: model.BasicEventElement, tag: str = NS_AAS + "basicEventElement") -> etree._Element:
    ...
    if obj.min_interval is not None:
        et_basic_event_element.append(
            _generate_element(NS_AAS + "minInterval", text=model.datatypes.xsd_repr(obj.min_interval))
        )
    if obj.max_interval is not None:
        et_basic_event_element.append(
            _generate_element(NS_AAS + "maxInterval", text=model.datatypes.xsd_repr(obj.max_interval))
        )
...
def write_aas_xml_file(file: _generic.PathOrBinaryIO, data: model.AbstractObjectStore, **kwargs) -> None:
    """
