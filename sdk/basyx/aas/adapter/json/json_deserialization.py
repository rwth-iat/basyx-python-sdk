...
class AASFromJsonDecoder(json.JSONDecoder):
    ...
    def _construct_key(cls, dct: Dict[str, object], object_class=model.Key) -> model.Key:
        return object_class(type_=KEY_TYPES_INVERSE[_get_ts(dct, "type", str)], value=_get_ts(dct, "value", str))
    ...
    def _construct_specific_asset_id(
        ...
        return object_class(
            ...
            external_subject_id=cls._construct_external_reference(_get_ts(dct, "externalSubjectId", dict))
            if "externalSubjectId" in dct
            else None,
            semantic_id=cls._construct_reference(_get_ts(dct, "semanticId", dict)) if "semanticId" in dct else None,
            supplemental_semantic_id=[
                cls._construct_reference(ref) for ref in _get_ts(dct, "supplementalSemanticIds", list)
            ]
            if "supplementalSemanticIds" in dct
            else (),
        )
    ...
    def _construct_external_reference(
        ...
        return object_class(
            tuple(keys),
            cls._construct_reference(_get_ts(dct, "referredSemanticId", dict)) if "referredSemanticId" in dct else None,
        )
    ...
    def _construct_value_reference_pair(
        ...
        return object_class(
            value=_get_ts(dct, "value", str),
            value_id=cls._construct_reference(_get_ts(dct, "valueId", dict)) if "valueId" in dct else None,
        )
    ...
    def _construct_entity(cls, dct: Dict[str, object], object_class=model.Entity) -> model.Entity:
        ...
        if "entityType" in dct:
            ...
            entity_type = None
        ret = object_class(
            id_short=None, entity_type=entity_type, global_asset_id=global_asset_id, specific_asset_id=specific_asset_id
        )
        cls._amend_abstract_attributes(ret, dct)
    ...
    def _construct_qualifier(cls, dct: Dict[str, object], object_class=model.Qualifier) -> model.Qualifier:
        ret = object_class(
            type_=_get_ts(dct, "type", str), value_type=model.datatypes.XSD_TYPE_CLASSES[_get_ts(dct, "valueType", str)]
        )
        cls._amend_abstract_attributes(ret, dct)
        if "value" in dct:
    ...
    def _construct_blob(cls, dct: Dict[str, object], object_class=model.Blob) -> model.Blob:
        ret = object_class(
            id_short=None, content_type=_get_ts(dct, "contentType", str) if "contentType" in dct else None
        )
        cls._amend_abstract_attributes(ret, dct)
        if "value" in dct:
    ...
    def _construct_file(cls, dct: Dict[str, object], object_class=model.File) -> model.File:
        ...
        ret = object_class(
            id_short=None, value=None, content_type=_get_ts(dct, "contentType", str) if "contentType" in dct else None
        )
        cls._amend_abstract_attributes(ret, dct)
