...
class AASDataCheckerTest(unittest.TestCase):
    def test_qualifiable_checker(self):
        qualifier_expected = model.Qualifier(type_="test", value_type=model.datatypes.String, value="test value")
        property_expected = model.Property(
            id_short="Prop1", value_type=model.datatypes.String, value="test", qualifier={qualifier_expected}
        )
    ...
    def test_submodel_element_list_checker(self):
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute max of Range[test_list[0]] must be == 1337 (value=142857)", repr(next(checker_iterator))
        )
        self.assertEqual(
            "FAIL: Attribute max of Range[test_list[1]] must be == 142857 (value=1337)", repr(next(checker_iterator))
        )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute order_relevant of SubmodelElementList[test_list] must be == True (value=False)",
            repr(next(checker_iterator)),
        )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute value_type_list_element of SubmodelElementList[test_list] must be == str (value='Int')",
            repr(next(checker_iterator)),
        )
    ...
    def test_submodel_element_collection_checker(self):
        ...
        property_expected = model.Property(id_short="Prop1", value_type=model.datatypes.String, value="test")
        range_expected = model.Range(id_short="Range1", value_type=model.datatypes.Int, min=100, max=200)
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute value of SubmodelElementCollection[Collection] must contain 2 SubmodelElements (count=1)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Submodel Element Property[Collection.Prop1] must exist ()", repr(next(checker_iterator))
        )
    ...
    def test_annotated_relationship_element(self):
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute annotation of AnnotatedRelationshipElement[test] must contain 1 DataElements (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Annotation Property[test.ExampleAnnotatedProperty] must exist ()", repr(next(checker_iterator))
        )
    ...
    def test_submodel_checker(self):
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute submodel_element of Submodel[test] must contain 1 SubmodelElements (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual("FAIL: Submodel Element Property[test / Prop1] must exist ()", repr(next(checker_iterator)))
    ...
    def test_asset_administration_shell_checker(self):
        ...
        shell_expected = model.AssetAdministrationShell(
            ...
            id_="test",
            submodel={model.ModelReference((model.Key(type_=model.KeyTypes.SUBMODEL, value="test"),), model.Submodel)},
        )
        checker = AASDataChecker(raise_immediately=False)
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute submodel of AssetAdministrationShell[test] must contain 1 ModelReferences (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Submodel Reference ModelReference<Submodel>(key=(Key(type=SUBMODEL, value=test),)) must exist ()",
            repr(next(checker_iterator)),
        )
    ...
    def test_concept_description_checker(self):
        ...
        cd_expected = model.ConceptDescription(
            id_="test",
            is_case_of={model.ExternalReference((model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="test"),))},
        )
        checker = AASDataChecker(raise_immediately=False)
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute is_case_of of ConceptDescription[test] must contain 1 References (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
