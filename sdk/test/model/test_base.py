...
def generate_example_referable_tree() -> model.Referable:
    ...
    def generate_example_referable_with_namespace(
        ...
        if child:
            namespace_set = model.NamespaceSet(parent=referable, attribute_names=[("id_short", True)], items=[child])
        return referable
...
class ReferableTest(unittest.TestCase):
    ...
    def test_representation(self):
        ...
        with self.assertRaises(AttributeError) as cm:
            ref.__repr__()
        self.assertEqual(
            "Referable must have an identifiable as root object and only parents that are referable", str(cm.exception)
        )
    ...
    def test_get_id_short_path(self):
        ...
        MySubSubmodelElementList1 = model.SubmodelElementList(
            "MySubSubmodelElementList1", model.Property, value_type_list_element=model.datatypes.String
        )
...
class ModelNamespaceTest(unittest.TestCase):
    ...
    def setUp(self):
        self.propSemanticID = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://example.org/Test1"),)
        )
        self.propSemanticID2 = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://example.org/Test2"),)
        )
        self.propSemanticID3 = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://example.org/Test3"),)
        )
    ...
    def test_namespaceset_hooks(self) -> None:
        ...
        for nss_type in nss_types:
            ...
            dummy_ns = DummyNamespace(
                {cap}, item_add_hook=add_hook, item_id_set_hook=id_set_hook, item_id_del_hook=id_del_hook
            )
            ...
            dummy_ns = DummyNamespace(
                (), item_add_hook=add_hook_constraint, item_id_set_hook=id_set_hook, item_id_del_hook=id_del_hook
            )
            add_hook_counter = 0
    ...
    def test_Namespace(self) -> None:
        ...
        self.assertEqual(
            f"'Referable with id_short Prop3 not found in {self._namespace_class.__name__}[{self.namespace.id}]'",
            str(cm2.exception),
        )
        ...
        self.assertEqual(
            f"'Referable with id_short Prop2 not found in {self._namespace_class.__name__}[{self.namespace.id}]'",
            str(cm3.exception),
        )
    ...
    def test_id_short_path_resolution(self) -> None:
        ...
        self.assertEqual(
            f"'Referable with id_short Prop2 not found in SubmodelElementCollection[{self.namespace.id} / List1[0]]'",
            str(cm_2.exception),
        )
    ...
    def test_renaming(self) -> None:
        ...
        self.assertEqual(
            f"'Referable with id_short Prop1 not found in {self._namespace_class.__name__}[{self.namespace.id}]'",
            str(cm.exception),
        )
        self.assertIs(self.prop2, self.namespace.get_referable("Prop2"))
    ...
    def test_qualifiable_id_short_namespace(self) -> None:
        ...
        submodel_element_collection = model.SubmodelElementCollection("test_SMC", [prop1], qualifier=[qualifier1])
...
class ModelOrderedNamespaceTest(ModelNamespaceTest):
    ...
    def test_OrderedNamespace(self) -> None:
        ...
        self.assertEqual(
            f"'Referable with id_short Prop1 not found in {self._namespace_class.__name__}[{self.namespace.id}]'",  # type: ignore[has-type]
            str(cm2.exception),
        )
...
class ModelReferenceTest(unittest.TestCase):
    def test_constraints(self):
        ...
        with self.assertRaises(model.AASConstraintViolation) as cm:
            model.ModelReference(keys, model.Property)
        self.assertEqual(
            f"{keys[-1]!r} is not preceded by a key of type File or Blob, but {keys[1]!r} (Constraint AASd-127)",
            str(cm.exception),
        )
    ...
    def test_equality(self):
        ...
        ref_2 = model.ModelReference(
            (model.Key(model.KeyTypes.SUBMODEL, "urn:x-test:x"), model.Key(model.KeyTypes.PROPERTY, "test")),
            model.Submodel,
        )
    ...
    def test_from_referable(self) -> None:
        ...
        self.assertEqual(
            "The given Referable object is not embedded within an Identifiable object", str(cm.exception).split(":")[0]
        )
...
class ValueReferencePairTest(unittest.TestCase):
    def test_set_value(self):
        pair = model.ValueReferencePair(
            value="2", value_id=model.ExternalReference((model.Key(model.KeyTypes.GLOBAL_REFERENCE, "test"),))
        )
...
class HasSemanticsTest(unittest.TestCase):
    def test_supplemental_semantic_id_constraint(self) -> None:
        ...
        self.assertEqual(
            "A semantic_id must be defined before adding a supplemental_semantic_id! (Constraint AASd-118)",
            str(cm.exception),
        )
...
class LangStringSetTest(unittest.TestCase):
    def test_language_tag_constraints(self) -> None:
        ...
        self.assertEqual(
            f"The language tag must follow the format defined in BCP 47. Given language tag: x", cm.exception.args[0]
        )

        with self.assertRaises(ValueError) as cm:
            model.LangStringSet({"foo-oo1": "bar"})
        self.assertEqual(
            f"The language tag must follow the format defined in BCP 47. Given language tag: foo-oo1",
            cm.exception.args[0],
        )
