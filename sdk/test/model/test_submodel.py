...
class EntityTest(unittest.TestCase):
    def test_aasd_014_init_self_managed(self) -> None:
        ...
        model.Entity(
            "TestEntity", model.EntityType.SELF_MANAGED_ENTITY, global_asset_id="https://example.org/TestAsset"
        )
        model.Entity(
            "TestEntity",
    ...
    def test_aasd_014_init_co_managed(self) -> None:
        ...
        with self.assertRaises(model.AASConstraintViolation) as cm:
            model.Entity(
                "TestEntity", model.EntityType.CO_MANAGED_ENTITY, global_asset_id="https://example.org/TestAsset"
            )
        self.assertEqual(
            "A co-managed entity has to have neither a globalAssetId nor a specificAssetId (Constraint AASd-014)",
            str(cm.exception),
        )
        with self.assertRaises(model.AASConstraintViolation) as cm:
            model.Entity(
...
class SubmodelElementListTest(unittest.TestCase):
    def test_constraints(self):
        ...
        mlp = model.MultiLanguageProperty(
            None,
            semantic_id=model.ExternalReference((model.Key(model.KeyTypes.GLOBAL_REFERENCE, "urn:x-test:invalid"),)),
        )
        with self.assertRaises(model.AASConstraintViolation) as cm:
        ...
        with self.assertRaises(ValueError) as cm:
            model.BasicEventElement(
                ...
                model.ModelReference(
                    (model.Key(model.KeyTypes.ASSET_ADMINISTRATION_SHELL, "urn:x-test:AssetAdministrationShell"),),
