...
request = urllib.request.Request(TEST_CONFIG["couchdb"]["url"], headers=default_headers, method="HEAD")
try:
...
request = urllib.request.Request(
    ...
    method="PUT",
    data=json.dumps(
        {"admins": {"names": [], "roles": []}, "members": {"names": [TEST_CONFIG["couchdb"]["user"]], "roles": []}}
    ).encode(),
)
urllib.request.urlopen(request)
